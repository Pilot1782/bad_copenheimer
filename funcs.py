...
class funcs:
  ...
  def login(host, self):
    ...
    for i in self.run_command("python3 {4}playerlist.pyw --auth {0}:{1} -p {2} {3}".format(usr_name,passwd,25565,host,home_dir)):
  ...
  def scan_range(ip1, ip2, self):
    ...
    with open(self.output_path) as fp:
      ...
      self.log("Successfully appended {0} lines to the JSON file".format(len(data)))